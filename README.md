...
SOFT_CORE.with_borrow_mut(|core| {
    core.csrrw(reg::X0, csr_name_map_backwards("mscratch").unsigned(), reg::X5)
        .unwrap();
    core.csrrs(reg::X5, csr_name_map_backwards("mepc").unsigned(), reg::X0)
        .unwrap();
    core.execute(ast::ITYPE((bv(4u64), reg::X5, reg::X5, iop::ADDI)));
    core.csrrw(reg::X0, csr_name_map_backwards("mepc").unsigned(), reg::X5)
        .unwrap();
    core.execute(ast::ITYPE((bv(1u64), reg::X0, reg::X5, iop::ADDI)));
    core.csrrw(reg::X5, csr_name_map_backwards("mscratch").unsigned(), reg::X5)
        .unwrap();
